...
class Sloth:
        ...
        if extensions:
            for extension_name, extension_config in extensions.items():
                try:
                    ...
                    ExtendedSloth = ext.extend_sloth(
                        ExtendedSloth,
    ...
    def handle(self, request):
        ...
        self.logger.debug(
            "Payload received from %s - %s"
            % (request.remote.ip, request.headers.get("User-Agent", ""))
        )
    ...
    def process(self, validator_params):
        ...
        if not self.queue_processor or not self.queue_processor.is_alive():
            self.queue_processor = Thread(
                target=self.process_queue, name=self.listen_point
            )
    ...
    def run_build(self, actions, params):
        ...
        for action in actions:
            try:
                ...
                if last_error and (
                    hasattr(action, "critical") or self.config.get("stop_on_first_fail")
                ):
        ...
        if not errors:
            ...
            self.build_logger.warning(
                "None completed: %d/%d" % (len(actions) - len(errors), len(actions))
            )

        else:
            self.build_logger.warning(
                "Partially completed: %d/%d"
                % (len(actions) - len(errors), len(actions))
            )
    ...
    def execute(self, action):
        ...
        try:
            ...
            stdout, stderr = process.communicate(
                timeout=self.config.get("exec_timeout")
            )
