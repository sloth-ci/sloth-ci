...
class Bed:
    ...
    def __new__(cls, config):
        ...
        api_extension = {"api": {"module": "api.bed"}}
    ...
    def _configure(self):
        ...
        access_log_path = self.config.get("paths", {}).get(
            "access_log", "sloth_access.log"
        )
        ...
        error_log_path = self.config.get("paths", {}).get(
            "error_log", "sloth_error.log"
        )
    ...
    def bind_to_file(self, listen_point, config_file):
        ...
        try:
            ...
            if sloth.config == load(open(config_file), Loader=FullLoader):
                ...
                cherrypy.log.error(
                    "App on %s bound with config file %s" % (listen_point, config_file)
                )
            ...
            cherrypy.log.error(
                "Failed to bind config file: listen point %s not found" % listen_point
            )
            raise

        except FileNotFoundError:
            cherrypy.log.error(
                "Failed to bind config file: file %s not found" % config_file
            )
            raise
    ...
    def create_from_config(self, config):
        ...
        if listen_point in self.sloths:
            cherrypy.log.error(
                "Failed to create app: listen point %s already taken" % listen_point
            )
    ...
    def remove(self, listen_point):
        ...
        try:
            ...
            cherrypy.log.error(
                "Failed to remove app: listen point %s not found" % listen_point
            )
            raise
