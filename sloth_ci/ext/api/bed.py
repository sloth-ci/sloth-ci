def extend_bed(cls, extension):
    ...
    class Bed(cls):
        ...
        def _setup_routing(self):
            ...
            for alias in ("auth", "api_auth"):
                ...
                if auth:
                    try:
                        auth_dict = {
                            entry["login"]: entry["password"] for entry in auth
                        }
        ...
        def _api_listener(self, action, **kwargs):
            ...
            cherrypy.request.error_page = {
                "default": lambda status, message, traceback, version: message
            }
        ...
        def bind(self, kwargs):
            ...
            try:
                ...
                raise cherrypy.HTTPError(
                    500, "Failed to bind config file to app: %s" % e
                )
        ...
        def create(self, kwargs):
            ...
            try:
                listen_point = self.create_from_config(
                    load(config_string, Loader=FullLoader)
                )
                ...
                raise cherrypy.HTTPError(
                    500, '"%s" is not a valid config string' % config_string
                )

            except KeyError as e:
                raise cherrypy.HTTPError(
                    500, 'The "%s" param is missing in the config' % e
                )
        ...
        def history(self, kwargs):
            ...
            if not self.db_path:
                raise cherrypy.HTTPError(
                    501,
                    "This Sloth server doesn't have a database to store build history",
                )
            ...
            try:
                ...
                history = [
                    dict(zip(column_names, record)) for record in cursor.fetchall()
                ]
        ...
        def info(self, kwargs):
            ...
            try:
                ...
                info = {"config_file": self.config_files.get(listen_point)}

                if self.db_path:
                    last_build_info = self.history(
                        {"listen_point": listen_point, "per_page": 1}
                    )
        ...
        def restart(self, kwargs):
            ...
            try:
                ...
                raise cherrypy.HTTPError(
                    500, "Failed to restart Sloth CI server: %s" % e
                )
        ...
        def trigger(self, kwargs):
            ...
            try:
                params = {
                    key: kwargs[key]
                    for key in kwargs
                    if key not in ("listen_point", "wait")
                }
                ...
                if not kwargs.get("wait"):
                    ...
                    success_url, fail_url = kwargs.get("success_url"), kwargs.get(
                        "fail_url"
                    )
                    ...
                    if build_successful and success_url:
                        ...
                        return self.info({"listen_point": listen_point})[
                            "last_build_status_message"
                        ]
        ...
        def version(self, kwargs):
            ...
            try:
                ...
                raise cherrypy.HTTPError(
                    500, "Failed to get Sloth CI server version: %s" % e
                )
