...
Run :ref:`sci start <cli-sci-start>` in this directory, and Sloth CI will start as a daemon on http://localost:8080:
...
This is a minimal :doc:`app config <app-config>`. Create the app from it with :ref:`sci create <cli-sci-create>`:
...
GitHub will send a ping event to verify that your app is properly configured. Run :ref:`sci logs timestamper <cli-sci-logs>`. The last record should be ``Valid payload received``—this means that your app successfuly received the ping payload from GitHub:
...
To apply the changes in the config, reload it with :ref:`sci reload timestamper <cli-sci-reload>`:
