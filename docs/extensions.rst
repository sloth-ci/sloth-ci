...
One :doc:`app config <ref/app-config>` can have many extensions; moreover, it can have the same extension used many times. For example, you can use the :mod:`File Logs <sloth_ci.ext.file_logs>` extension to write the error log and use the same extension to write the debug log into a different location.

Server-level extensions are invoked in the :doc:`server config <ref/server-config>`. These extensions change the way Sloth CI server works. For example, the :mod:`Robots.txt <sloth_ci.ext.robots_txt>` extension protects the server from bots; this doesn't affect any particular app but affects the whole server.
...
.. seealso::
    ...
    :ref:`API <apidocs-ext-api>` and :ref:`DB <apidocs-ext-db>` extensions are shipped with Sloth CI and are documented in the :doc:`API docs <ref/apidocs>`.
